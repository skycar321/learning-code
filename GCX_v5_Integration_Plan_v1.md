...
**검증**:
```bash
python .gcx/lib/test_ai_invoker.py
# Expected: Codex 호출 성공, 한글 출력 확인
```

#### Step 1.3: Validator 모듈
**파일**:
- `.gcx/lib/validators.py`
- `.gcx/lib/preflight.py`
...
**검증**:
```bash
python .gcx/cli.py preflight --fix
...
**파일**:
- `.gcx/hooks/user_prompt.py`
- `.gcx/hooks/pre_bash.py`
- `.gcx/hooks/post_bash.py`
- `.gcx/hooks/subagent_stop.py`
...
```bash
# Hook 테스트
echo '{"user_message": "/gcx-project test"}' | python .gcx/hooks/user_prompt.py
...
**검증 스크립트**: `.gcx/tests/test_e2e_gcx_project.sh`
```bash
#!/usr/bin/env zsh
...
### 1. `.gcx/lib/ai_invokers.py` (최우선 구현)
...
- 기존 v4 스크립트: `C:/Users/Nam/Desktop/Workspace/learning-code/.gcx/templates/` (참고용)
