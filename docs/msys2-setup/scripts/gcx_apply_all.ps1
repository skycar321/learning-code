...
$logPath = Join-Path $logDir ("gcx_apply_all_$ts.log")
...
# 3) MSYS2 optimizations
$bash = 'C:\\msys64\\usr\\bin\\bash.exe'
$opt = Join-Path $root 'docs\\msys2-setup\\scripts\\gcx_msys2_optimize.sh'
if ((Test-Path $bash) -and (Test-Path $opt)) {
    $includeInstall = $env:GCX_INCLUDE_INSTALL
    if (-not $includeInstall) { $includeInstall = '1' }
    $cmd = "`"$bash`" -lc `"GCX_INCLUDE_INSTALL=$includeInstall '$opt'`""
    Run-Logged 'gcx_msys2_optimize' $cmd
}
